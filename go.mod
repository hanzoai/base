...
// go-sqlite3 v2.0.3 is an invalid phantom tag (v2 without a /v2 module path),
// pulled only via psanford/sqlite3vfs's test deps (through hanzoai/replicate)
// and never actually built. Redirect every version to the last real release so
// module-graph resolution succeeds. Remote version pin — NOT a local replace.
