...
// the plugin: `waitlists`, `waitlist_entries`, `waitlist_events`. Safe on every
// boot — an existing pre-points list gains the points/breakdown fields and the
// points rank index without losing data.
...
func (p *plugin) ensureEntriesCollection() (*core.Collection, error) {
	...
	// Reconcile the points fields (present on new collections; added in place on
	// an existing pre-points list). Points is the single number position derives
	// from; breakdown holds the per-source subtotals the status view renders.
...
func (p *plugin) ensureEventsCollection(entries *core.Collection) (*core.Collection, error) {
	...
	c.Fields.Add(
		...
		// = never deduped (e.g. an explicit admin grant).
