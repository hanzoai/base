...
// TestBoostThroughSeam proves the service-authed /boost credits a caller-supplied
// amount through the SAME award() choke point as everything else — one points
// number, no parallel rank-boost store — and that it is service-gated,
// accumulates by default, and is replay-safe when keyed.
func TestBoostThroughSeam(t *testing.T) {
	p, app, cleanup := newPointsTestPlugin(t)
	defer cleanup()
	srv := serve(t, p, app)
	defer srv.Close()
	...
	do(t, "POST", base+"/join", "", map[string]any{"waitlist": "demo", "email": "node@example.com"})

	// unauthenticated boost -> 401 (AdminSecret is configured).
	resp, _ := do(t, "POST", base+"/boost", "", map[string]any{
		"waitlist": "demo", "email": "node@example.com", "source": "hanzod", "amount": 40,
	})
	if resp.StatusCode != http.StatusUnauthorized {
		t.Fatalf("boost without auth = %d, want 401", resp.StatusCode)
	}

	// service-authed boost credits the caller amount through the award seam and
	// can flip the sticky access grant.
	_, b := do(t, "POST", base+"/boost", testAdminSecret, map[string]any{
		"waitlist": "demo", "email": "node@example.com", "source": "hanzod", "amount": 40, "grantAccess": true,
	})
	if num(t, b, "awarded") != 40 || num(t, b, "points") != 40 {
		t.Fatalf("boost = %v, want awarded 40 points 40", b)
	}
	if hasAccess, _ := b["hasAccess"].(bool); !hasAccess {
		t.Fatalf("boost grantAccess should grant access: %v", b)
	}

	// One number, one walk: /status reflects the same points and breakdown — no
	// parallel store to reconcile.
	_, st := do(t, "GET", base+"/status?waitlist=demo&email=node@example.com", "", nil)
	if num(t, st, "points") != 40 {
		t.Fatalf("status points after boost = %d, want 40", num(t, st, "points"))
	}
	if got := breakdown(t, st, "hanzod"); got != 40 {
		t.Fatalf("hanzod breakdown = %d, want 40", got)
	}
	if hasAccess, _ := st["hasAccess"].(bool); !hasAccess {
		t.Fatalf("status must show sticky access after boost grant: %v", st)
	}

	// Empty dedupKey is an accumulator — a second boost adds again.
	_, b2 := do(t, "POST", base+"/boost", testAdminSecret, map[string]any{
		"waitlist": "demo", "email": "node@example.com", "source": "hanzod", "amount": 10,
	})
	if num(t, b2, "points") != 50 {
		t.Fatalf("boost accumulate = %d, want 50", num(t, b2, "points"))
	}

	// A keyed boost is replay-safe: same dedupKey twice = one award.
	_, k1 := do(t, "POST", base+"/boost", testAdminSecret, map[string]any{
		"waitlist": "demo", "email": "node@example.com", "source": "hanzod:run", "amount": 25, "dedupKey": "run-7",
	})
	if num(t, k1, "awarded") != 25 || num(t, k1, "points") != 75 {
		t.Fatalf("keyed boost = %v, want awarded 25 points 75", k1)
	}
	_, k2 := do(t, "POST", base+"/boost", testAdminSecret, map[string]any{
		"waitlist": "demo", "email": "node@example.com", "source": "hanzod:run", "amount": 25, "dedupKey": "run-7",
	})
	if num(t, k2, "awarded") != 0 || !k2["alreadyAwarded"].(bool) || num(t, k2, "points") != 75 {
		t.Fatalf("keyed boost replay = %v, want deduped points 75", k2)
	}

	// unknown source rejected.
	resp3, _ := do(t, "POST", base+"/boost", testAdminSecret, map[string]any{
		"waitlist": "demo", "email": "node@example.com", "source": "bogus", "amount": 5,
	})
	if resp3.StatusCode != http.StatusBadRequest {
		t.Fatalf("bogus boost source = %d, want 400", resp3.StatusCode)
	}
}
