...
// initCreatedField ensures every base/auth collection carries a system `created`
// autodate field. Without it a record set can't be sorted by recency — the
// admin UI's default `?sort=-created` 400s ("invalid sort field") on any
// user-created collection, and a CRM/CMS list can't order by newest. Matches the
// PocketBase heritage + the reference CRM (every object gets a created stamp).
func (c *Collection) initCreatedField() {
	if c.Fields.GetByName(FieldNameCreated) != nil {
		return
	}
	c.Fields.Add(&AutodateField{
...
// initUpdatedField ensures every base/auth collection carries a system `updated`
// autodate field (stamped on create and every update).
func (c *Collection) initUpdatedField() {
	if c.Fields.GetByName(FieldNameUpdated) != nil {
		return
	}
	c.Fields.Add(&AutodateField{
