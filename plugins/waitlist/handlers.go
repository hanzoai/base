...
func (p *plugin) handleExport(e *core.RequestEvent) error {
	...
	w := csv.NewWriter(&buf)
	_ = w.Write([]string{"rank", "email", "refCode", "referredBy", "points", "referralCount", "createdAt"})
...
// entryView builds the join/status wire object for a single entry.
...
// validAwardSource gates /award to the known earn channels (open-ended within
// social:/hanzod so a new network needs no core change).
