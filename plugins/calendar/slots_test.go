...
func TestComputeSlots(t *testing.T) {
	...
	got := computeSlots(from, to, now, 30, 0, windows, loc, nil, 0)
	if len(got) != 4 {
	...
	got = computeSlots(from, to, now, 30, 0, windows, loc, busy, 0)
	if len(got) != 3 {
	...
	got = computeSlots(from, to, sameDayNow, 30, 0, windows, loc, nil, 0)
...
func TestComputeSlotsGuards(t *testing.T) {
	...
	from := time.Date(2026, 7, 27, 0, 0, 0, 0, loc)
	to := from.Add(24 * time.Hour)
	if s := computeSlots(from, to, from, 0, 0, []availWindow{{Weekday: 1, StartMinute: 0, EndMinute: 600}}, loc, nil, 0); s != nil {
		t.Errorf("zero duration must yield no slots, got %v", s)
	}
	if s := computeSlots(from, to, from, 30, 0, nil, loc, nil, 0); s != nil {
...
func TestComputeSlotsRejectsOffWindow(t *testing.T) {
	...
	windows := []availWindow{{Weekday: wd, StartMinute: 9 * 60, EndMinute: 11 * 60}}
	slots := computeSlots(from, to, now, 30, 0, windows, loc, nil, 0)
