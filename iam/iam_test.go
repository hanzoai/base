...
func TestLookupByAttribute_Hit(t *testing.T) {
	...
	f.setHandler("/api/get-users", func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.URL.Query().Get("owner"); got != "acme" {
			t.Errorf("owner: got %q want acme", got)
		}
		writeOK(w, []map[string]any{
			{"id": "u-1", "name": "alice", "email": "alice@x.com", "owner": "acme"},
		})
	})

	c := iam.NewClient(f.server.URL)
	c.SetAdminCreds(iam.AdminCreds{ClientID: "svc", ClientSecret: "shh", Owner: "acme"})
...
func TestLookupByAttribute_PhoneNormalization(t *testing.T) {
	...
	f.setHandler("/api/get-users", func(w http.ResponseWriter, r *http.Request) {
		...
		case "6125551234":
			writeOK(w, []map[string]any{
				{"id": "u-7", "name": "bob", "email": "bob@x.com", "owner": "acme"},
			})
		default:
			writeOK(w, []map[string]any{})
		}
...
func TestLookupByAttribute_RequiresAdminCreds(t *testing.T) {
	...
	_, err := c.LookupByAttribute(context.Background(), "email", "x@y.com", "acme", 10)
...
func TestEnsureUser_Create(t *testing.T) {
	...
	f.setHandler("/api/add-user", func(w http.ResponseWriter, r *http.Request) {
		...
		if payload["owner"] != "acme" {
			t.Errorf("owner: got %v want acme", payload["owner"])
		}
		writeOK(w, "Affected")
	})
