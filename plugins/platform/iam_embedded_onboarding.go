...
// IAM. Mirrors the shape of liquidityio/id's 5-step BD pipeline so
...
//
//   IDV_PROVIDER=onyxplus | persona | jumio | hyperverge | custom | none
//   IDV_PROVIDER_URL=…
...
// onyxPlusAdapter is the canonical reference adapter, mirroring the
// liquidityio/id integration. Real implementation lives in a sibling
// commit; the interface lands first so frontends and ops manifests
// can pin against it.
type onyxPlusAdapter struct {
	baseURL string
	apiKey  string
}

func idvOnyxPlusFromEnv() IDVProvider {
	return &onyxPlusAdapter{
		baseURL: os.Getenv("IDV_PROVIDER_URL"),
		apiKey:  os.Getenv("IDV_PROVIDER_API_KEY"),
	}
}

func (a *onyxPlusAdapter) Name() string { return "OnyxPlus" }
func (a *onyxPlusAdapter) StartSession(ctx context.Context, req IDVStartReq) (*IDVSession, error) {
	// TODO: POST to ${baseURL}/v1/onyx/enrollments — wire after the
	// onboarding endpoints land. The contract here is stable.
	return nil, errors.New("onyxplus adapter not yet wired")
}
func (a *onyxPlusAdapter) FetchAttestation(ctx context.Context, sessionID string) (*IDVAttestation, error) {
