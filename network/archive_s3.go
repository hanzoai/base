...
// s3Archive speaks the S3 API. It works against hanzoai/s3 self-hosted,
// AWS S3, and any other S3-compatible endpoint.
//
...
//
// Endpoint resolution:
//   - AWS_ENDPOINT_URL set      → that endpoint (hanzoai/s3).
...
func (u *s3Upload) put(ctx context.Context, key string, body []byte) error {
	_, err := u.client.PutObject(ctx, u.bucket, key, bytes.NewReader(body), int64(len(body)), s3.PutObjectOptions{
...
func (u *s3Upload) get(ctx context.Context, key string) ([]byte, error) {
	obj, err := u.client.GetObject(ctx, u.bucket, key, s3.GetObjectOptions{})
	if err != nil {
...
func (u *s3Upload) list(ctx context.Context, prefix string) ([]string, error) {
	var out []string
	for info := range u.client.ListObjects(ctx, u.bucket, s3.ListObjectsOptions{
		Prefix:    prefix,
		Recursive: true,
	}) {
		if info.Err != nil {
...
// AWS_ENDPOINT_URL lets operators point at hanzoai/s3 or any other
// S3-compatible endpoint; without it, we talk to AWS.
...
// Identity via the client's credentials.IAM provider.
