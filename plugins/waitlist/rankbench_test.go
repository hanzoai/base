...
func TestPointsScaleBenchmark(t *testing.T) {
	...
	if testing.Short() {
		// -short keeps the bench a fast smoke test of the exact SQL mechanics:
		// one small size. The 10M run is reserved for an explicit full
		// (non-short) invocation, e.g. WL_BENCH=1 WL_BENCH_N=10000000 go test.
		sizes = []int{50_000}
	}
