...
func TestResolveWriterURL(t *testing.T) {
	eps := map[string]string{
		"liquid-bd-0.liquid-bd-network.example.svc:9999": "http://bd-0.internal:8090",
	}
	...
	cases := []struct {
		...
		{"empty-owner", "", "", ""},
		{"explicit-map", "liquid-bd-0.liquid-bd-network.example.svc:9999", "http://bd-0.internal:8090", ""},
		{"derived-default-port", "liquid-bd-1.liquid-bd-network.example.svc:9999", "http://liquid-bd-1.liquid-bd-network.example.svc:8090", ""},
...
func TestWriteForwardConventionDerivedURL(t *testing.T) {
	n := &fakeNet{
		owner: "liquid-bd-2.liquid-bd-network.example.svc:9999",
		local: false,
	}
	fn := writeForward(n, nil)
	...
	loc := rec.Header().Get("Location")
	wantPrefix := "http://liquid-bd-2.liquid-bd-network.example.svc:8090"
